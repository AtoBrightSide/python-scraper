...
class ThirteenFScraper:
        ...
        async with session.get(self.managers_url) as response:
            ...
            for tr in tbody.find_all("tr"):
                ...
                if len(cells) >= 3:
                    ...
                    manager_url = (self.base_url.rstrip("/") + a_tag['href']
                                   if (a_tag and a_tag.get('href')) else None)
                    managers.append(Manager(manager_name, manager_url))
            return managers

    async def get_filings_for_manager(self, manager: Manager, session: aiohttp.ClientSession):
        """
    ...
    async def fetch_all_holdings(self, manager: Manager, session: aiohttp.ClientSession):
        """
        For a given manager, fetch holdings for all filings concurrently.
        Returns a tuple (holdings_by_filing, failed_records) where:
          - holdings_by_filing: a dictionary mapping Filing objects to their holdings lists.
          - failed_records: a list of dictionaries, each corresponding to a filing whose holdings failed.
        """
        tasks = []
        for filing in manager.filings:
            tasks.append(self.api_client.fetch_holdings(
                filing.filing_id, session))
        ...
        for filing, res in zip(manager.filings, results):
            if isinstance(res, Exception):
                print(
                    f"Error fetching holdings for filing {filing.filing_id}: {res}")
                ...
                holdings_by_filing[filing] = res

        return holdings_by_filing, failed_records
    ...
    async def run(self, output_filename='./data/final.csv'):
        ...
        Main pipeline:
          ...
          5. Write the final CSV and log failed records(if there any).
        """
        failed_records_total = []
        async with aiohttp.ClientSession(headers={'User-Agent': 'Mozilla/5.0 (compatible; DataScraper/1.0)'}) as session:
            managers = await self.get_managers(session)
            # Fetch filings in parallel for all managers.
            get_filings_tasks = [self.get_filings_for_manager(
                manager, session) for manager in managers]
            ...
            for manager in managers:
                ...
                holdings_by_filing, failed_records = await self.fetch_all_holdings(manager, session)
                failed_records_total.extend(failed_records)
                for filing in manager.filings:
                    holdings = holdings_by_filing.get(filing)
                    if not holdings:
                        continue
            ...
            df['pct_change'] = (
                df['change'] / df['prev_shares'] * 100).round(2)
            ...
            print(f"Total failed holdings: {len(failed_records_total)} / {df}")
